...
import cn.sincerity.sharebuy.infrastructure.properties.JwtProperties;
...
public class TokenProvider {
    ...
    public AccessToken createToken(String subject) {
        ...
        // 过期时间
        final Date expirationDate = new Date(now.getTime() + jwtProperties.getExpiration() * 1000);

        String token = jwtProperties.getTokenHead() + Jwts.builder()
                .setSubject(subject)
                .setIssuedAt(now)
    ...
    public AccessToken refreshToken(String oldToken) {
        String token = oldToken.substring(jwtProperties.getHeader().length());
